...
public class ImportAudioService extends AbstractExecutionThreadService
{
	 ...
	 */
	private Pattern PROGRESS_PATTERN = Pattern
			.compile("\\[download\\]\\s*([0-9\\.]*)%\\s*of\\s*([0-9\\.a-zA-Z]*)\\s*at\\s*([0-9\\.a-zA-Z/]*).*");
	...
	{
		// Starting YouTube-DL
		String output = DirectoryUtil.getImportAudioDirectory().getAbsolutePath() + File.separator
				+ "%(title)s.%(ext)s";
		String command = "youtube-dl -v --prefer-ffmpeg --encoding UTF-8 --newline -f bestaudio -x --audio-format "
				+ importAudio.getFormat() + " --audio-quality " + importAudio.getQuality() + " -o";
	...
	/**
	 * Return the version of Youtube-DL. Return null if Youtube-DL is not present.
	 * 
	...
	/**
	 * Return the version of FFMPEG. Return null if FFMPEG is not present.
	 * 
	...
	/**
	 * Return a copied version of the currently importing audio from external
	 * sources.
	 * 
	...
	public void cleanup()
	{
		synchronized (importAudioList)
		{
			for (Iterator<ImportAudio> it = importAudioList.iterator(); it.hasNext();)
			{
				...
				if (importAudio.getStatus() == ImportAudio.Status.DONE
						|| importAudio.getStatus() == ImportAudio.Status.ERROR)
				{
					it.remove();
				}
			}
			...
			for (ImportAudio importAudio : importAudioList)
			{
				if (importAudio.getId().equals(id))
				{
					...
					importAudioQueue.add(
							new ImportAudio(importAudio.getUrl(), importAudio.getQuality(), importAudio.getFormat()));
					return;
				}
			}
		}
	...
	{
		try (ZipArchiveInputStream archiveInputStream = new ZipArchiveInputStream(new FileInputStream(file),
				Charsets.UTF_8.name()))
		{
			...
			while (archiveEntry != null)
			{
				...
				if (Constants.SUPPORTED_AUDIO_EXTENSIONS.contains(archiveExt))
				{
					...
					File outputFile = new File(
							importDir + File.separator + new File(archiveEntry.getName()).getName());
	...
	public List<ImportAudioFile> getImportedFileList()
	{
		...
		synchronized (importAudioList)
		{
			for (Iterator<File> it = fileList.iterator(); it.hasNext();)
			{
				...
				for (ImportAudio importAudio : importAudioList)
				{
					if (importAudio.getStatus() == Status.INPROGRESS
							&& importAudio.getWorkingFiles().contains(file.getName()))
					{
						it.remove();
					}
				}
	...
	{
		return fileList.stream().map(this::guessTags).collect(Collectors.toList());
	}
	...
	private ImportAudioFile guessTags(File file)
	{
		...
		try
		{
			...
			importAudioFile
					.setTitle(StringUtils.substring(StringUtils.trimToEmpty(tag.getFirst(FieldKey.TITLE)), 0, 2000));

			// Read track artist (can be empty string)
			importAudioFile
					.setArtist(StringUtils.substring(StringUtils.trimToEmpty(tag.getFirst(FieldKey.ARTIST)), 0, 1000));
	 ...
	 */
	public void tagFile(final String fileName, final Integer order, final String title, final String album,
			final String artist, final String albumArtist, final Directory directory) throws Exception
	{
		syncExecutor.submit((Callable<Void>) () -> {
			// Retrieve the file from imported files
			List<ImportAudioFile> importedFileList = AppContext.getInstance().getImportAudioService()
					.getImportedFileList();
			ImportAudioFile importAudioFile = importedFileList.stream()
					.filter(e -> e.getFile().getName().equals(fileName)).findFirst()
			...
			String extension = Files.getFileExtension(fileName);
			Path path = Paths.get(directory.getLocation(), albumDirectory,
